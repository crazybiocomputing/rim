...
impl  FileInfo  {
    ...
    const int GRAY16_SIGNED = 1;
    
    /** 16-bit unsigned integer (0-65535). */
    const int GRAY16_UNSIGNED = 2;
    
    /**    32-bit signed integer. Imported 32-bit integer images are
        converted to floating-point. */
    const int GRAY32_INT = 3;
    
    /** 32-bit floating-point. */
    const int GRAY32_FLOAT = 4;
    ...
    const int RAW = 1;
    const int TIFF = 2;
    const int GIF_OR_JPG = 3;
    const int FITS = 4;
    const int BMP = 5;
    const int DICOM = 6;
    const int ZIP_ARCHIVE = 7;
    const int PGM = 8;
    const int IMAGEIO = 9;

    // Compression modes
    const int COMPRESSION_UNKNOWN = 0;
    const int COMPRESSION_NONE= 1;
    const int LZW = 2;
    const int LZW_WITH_DIFFERENCING = 3;
    const int JPEG = 4;
    const int PACK_BITS = 5;
    const int ZIP = 6;
